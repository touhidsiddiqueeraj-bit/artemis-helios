...
%% FIX: movmean replaced with conv() for compatibility with MATLAB < R2016a
...
%% FIX: conv() replaces movmean() — compatible with all MATLAB versions
win      = round(1800 / dt);
kernel   = ones(win, 1) / win;
eta_roll = conv(eta_inst, kernel, 'same');
...
% Super-title: compatible with all MATLAB versions (no sgtitle, no text(ax,...))
ha_ax = findall(gcf,'Type','axes');
for ha_k = 1:numel(ha_ax)
    ha_p = get(ha_ax(ha_k),'Position');
    if ha_p(4) > 0.05
        set(ha_ax(ha_k),'Position',[ha_p(1),ha_p(2),ha_p(3),ha_p(4)*0.92]);
    end
end
axes('Position',[0 0.95 1 0.04],'Visible','off');
text(0.5, 0.5, 'Helios-Artemis MPPT v3  --  Single-Day Simulation',...
...
text(0.5, 152, 'Tj limit 150 degC','FontSize',8,'Color','k');
hold off;
ylabel('Junction Temp  (degC)');
...
ax7 = subplot(2,2,4);
scatter(log_ghi(mask), eta_inst(mask)*100, 2, [0.20 0.60 0.80], 'filled');
xlabel('GHI  (W/m^2)');
ylabel('MPPT eta  (%)');
...
axes('Position',[0 0.95 1 0.04],'Visible','off');
text(0.5, 0.5, 'Helios-Artemis MPPT v3  --  Battery & Thermal',...
     'FontSize',13,'FontWeight','bold',...
     'HorizontalAlignment','center','VerticalAlignment','middle',...
     'Units','normalized');
...
axes('Position',[0 0.95 1 0.04],'Visible','off');
text(0.5, 0.5, 'Helios-Artemis MPPT v3  --  Energy Summary',...
     'FontSize',13,'FontWeight','bold',...
     'HorizontalAlignment','center','VerticalAlignment','middle',...
     'Units','normalized');
...
figure('Name','HA-MPPT v3 - Daily Summary','Color','w',...
       'Position',[240 240 560 460]);
axes('Position',[0 0 1 1],'Visible','off');
xlim([0 1]); ylim([0 1]); hold on;

% Outer border
rectangle('Position',[0.04 0.04 0.92 0.92],...
          'FaceColor',[0.97 0.97 0.97],'EdgeColor',[0.65 0.65 0.65],...
          'LineWidth',1.5);

% Dark blue title bar
rectangle('Position',[0.04 0.84 0.92 0.12],...
          'FaceColor',[0.13 0.31 0.56],'EdgeColor','none');
text(0.50, 0.902, 'DAILY SUMMARY',...
     'Color','w','FontSize',14,'FontWeight','bold',...
     'HorizontalAlignment','center','VerticalAlignment','middle');

% Separator line
line([0.05 0.95],[0.84 0.84],'Color',[0.50 0.50 0.50],'LineWidth',1.0);

% Metric data
mData = {
    'Peak GHI',          sprintf('%.1f W/m2',  G_peak);
    'Peak PV Power',     sprintf('%.2f W',      P_peak);
    'Energy Harvested',  sprintf('%.2f Wh',     E_pv);
    'MPP Energy Avail',  sprintf('%.2f Wh',     E_mpp);
    'Daily MPPT eta',    sprintf('%.2f %%',     eta_day);
    'Battery SoC (end)', sprintf('%.1f %%',     SoC_end);
    'Battery Vt (end)',  sprintf('%.3f V',      Vb_end);
    'MOSFET Tj Peak',    sprintf('%.1f degC',   Tj_peak);
};
...
xL    = 0.07;
xDiv  = 0.58;
rowC1 = [1.00 1.00 1.00];
rowC2 = [0.92 0.95 1.00];
valC  = [0.08 0.25 0.55];

for i = 1:nR
    y0 = yTop - i*rowH;
    if mod(i,2)==0, rc=rowC2; else, rc=rowC1; end
    rectangle('Position',[0.05 y0 0.90 rowH],'FaceColor',rc,'EdgeColor','none');
    line([0.05 0.95],[y0 y0],'Color',[0.82 0.82 0.82],'LineWidth',0.5);
    text(xL, y0+rowH*0.5, mData{i,1},...
         'FontSize',10,'FontWeight','normal','Color',[0.15 0.15 0.15],...
         'HorizontalAlignment','left','VerticalAlignment','middle');
    text(0.93, y0+rowH*0.5, mData{i,2},...
         'FontSize',10,'FontWeight','bold','Color',valC,...
         'HorizontalAlignment','right','VerticalAlignment','middle');
end

% Vertical divider
line([xDiv xDiv],[0.06 0.84],'Color',[0.75 0.75 0.75],'LineWidth',0.8);
% Bottom border
line([0.05 0.95],[0.06 0.06],'Color',[0.65 0.65 0.65],'LineWidth',1.0);

hold off;

fprintf('Plots complete. (4 figures rendered)\n');